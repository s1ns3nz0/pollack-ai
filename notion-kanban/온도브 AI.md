...
- **Status**: To Do
- **URL**: https://app.notion.com/p/38df5e835bb4806b8afec7bf349961f0
...
Kill chain: 모델정찰 → 적대샘플 생성 → 패치/디코이 배치 → 인식 회피 → 임무 무력화. (검증 플래그: 전장 실사용은 실험실·연구 단계 확인, 실전 미확인 — references.md)
...
- **심각도 = 정책 엔진(LLM 아님):** m 등급, RAG/프롬프트 포이즈닝으로 못 낮춤(S5 저항성 동일 아키텍처). RAG 장애 시 빈 컨텍스트로 우아하게 강등

※ SITL엔 실 인식모델이 없어 합성 perception 스트림으로 검증. 스키마(EoClass 등)가 perception-tap NDJSON과 동일해, 실 온보드 추론 로그가 생기면 브리지/탐지기 무변경 교체 가능.

### ② 활용한 데이터

| 데이터 | 분류 | 용도 |
|---|---|---|
| RAG DB (126문서, bge-m3 임베딩) | 벡터화 지식 | Investigation 검색 근거(incident 13·MITRE ICS ~99·IEC62443·dataset 카드). Recall@5/MRR=1.0 |
...
데이터 흐름: 원천 raw(PCAP/CSV/XLSX) → 분석·특징추출 → 요약·시그니처·사건카드(.md) → 임베딩 → RAG DB. raw는 숫자행이라 RAG에 직접 임베드하지 않고, 추출한 요약/시그니처만 RAG화(의미검색 가능 형태로).
...
| LLM 분석 | '디코이 회피공격(AML.T0015·T0043)' 시사 — EO/IR 불일치·신뢰도 gap 0.46 |
| 판정/대응 | true_positive → response (LOITER→RTB 폐루프) |

- **온보드/엣지 방어 레이어(통신 두절에도 자율):** 메타 AI가 주 비전모델 출력·판단을 상시 감시(AI가 AI를 감시) → 적대 의심 시 경량 백업 비전모델 자동 전환 + 주 모델 격리

### MITRE 매핑
ATLAS AML.T0015(Evade ML Model) · AML.T0043(Craft Adversarial Data) / EMB3D 센서변조. (공격받는 AI를 ATLAS로 매핑 → S5 방어 AI와 함께 '양쪽 AI'를 커버)
...
파일은 Azure, **dahpollackstorage/onboardai 컨테이너에 Data 및 코드 저장**
