...
class _PendingOutcome:
    ...
    __slots__ = ("event", "records", "records_needed")

    def __init__(self, event: BridgeEvent, records_needed: int) -> None:
        self.event = event
    ...
    async def process(self, record: TelemetryRecord) -> BridgeEvent | None:
        ...
        if self._obs_source is not None:
            self._pending.append(
                _PendingOutcome(event, records_needed=self._outcome_window)
            )
        return event
