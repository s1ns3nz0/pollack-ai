...
class InvestigationAgent(BaseSOCAgent):
    ...
    async def run(self, state: SOCState) -> SOCState:
        ...
        # egress 정제: untrusted wire IOC 를 외부 조회 전 사설/내부/불정 드롭 + cap.
        # cap 은 alert당 단일 outbound 예산(wire+추출 공유) — 2×cap 우회 방지(Codex).
        cap = self._settings.ioc_egress_max_per_alert
        clean_iocs = alert.iocs
        if self._egress is not None:
            clean_iocs, egress_dropped = self._egress.sanitize(alert.iocs, cap=cap)
            if egress_dropped:
                self._logger.info(
                    "IOC egress 드롭 %d건(사설/내부/불정/cap): alert=%s",
                    len(egress_dropped),
                    alert.id,
                )

        # 샌드박스 디토네이션: 정제된 해시 IOC 를 터뜨려 행위 판정 + IOC 추출(데드라인).
        sandbox_reports = await self._bounded(self._detonate(clean_iocs), [], "sandbox")
        extracted = [
            ioc
        ...
        # 외부 TI 보강: wire+추출 IOC 를 단일 예산으로 재정제(추출도 오염방어)·조회.
        ti_indicators = [*clean_iocs, *extracted]
        if self._egress is not None:
            ti_indicators, _ = self._egress.sanitize(ti_indicators, cap=cap)
        else:
            ti_indicators = list(dict.fromkeys(ti_indicators))
        ...
        # 취약점 컨텍스트: 경보 CVE 의 악용여부(KEV)·심각도 보강(데드라인).
    ...
    async def _bounded(self, coro: Awaitable[_T], fallback: _T, label: str) -> _T:
        """enrich 호출을 벽시계 데드라인으로 감싼다(초과 시 fail-open 강등).

        per-request HTTP 타임아웃과 별개로, 직렬 다중 IOC 조회가 hotpath 를 무한정
        붙잡지 않게 한다(Codex High). 초과·취소 시 로깅 후 fallback 반환(비crash).

        Args:
            coro: 실행할 enrich 코루틴.
            fallback: 데드라인 초과 시 반환할 강등 값(보통 빈 리스트).
            label: 로깅용 enrich 이름.

        Returns:
            코루틴 결과, 또는 데드라인 초과 시 fallback.
        """
        try:
            return await asyncio.wait_for(
                coro, timeout=self._settings.enrichment_deadline_seconds
            )
        except TimeoutError:
            self._logger.warning(
                "enrich '%s' 데드라인(%.1fs) 초과 → fail-open 강등",
                label,
                self._settings.enrichment_deadline_seconds,
            )
            return fallback
