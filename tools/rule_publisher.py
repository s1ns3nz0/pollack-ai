...
- `GitHubRulePublisher` : 실 GitHub API 로 CSV 변경·브랜치·커밋·PR 생성(멱등).

원칙: KQL 룰은 절대 건드리지 않는다. PR 은 Watch List 파일 변경만 담는다. 변경은
회귀 게이트(benchmarks: 알려진 TP 무손실) 통과 후 머지하도록 본문에 명시한다.
"""
    ...
    async def apublish(self, pr: RulePullRequest) -> RulePullRequest:
        ...
        try:
            async with self._make_client() as client:
                ...
                new_text, changed = apply_watchlist(existing, pr.watchlist_update)
                if not changed:
