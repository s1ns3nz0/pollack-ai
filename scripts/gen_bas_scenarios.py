...
# UAV ATT&CK 15전술 — data/attack_coverage.yaml·CACAO 카탈로그와 공유하는 어휘.
# dah 원본은 Enterprise ATT&CK 명명이라 정규화(별칭 + UAV-15 우선 채택)가 필요.
_UAV15_TACTICS = {
    "Collection",
    "CommandAndControl",
    "Discovery",
...
def _pick_tactic(tactics: list[str]) -> str:
    """대표 전술 선택 — 별칭 정규화 후 첫 UAV-15 전술, 없으면 첫 값 유지.

    Args:
        tactics: dah 룰 JSON 의 tactics 배열(Enterprise 명명 가능).

    Returns:
        UAV ATT&CK 어휘로 정규화된 대표 전술. UAV-15 밖 전술만 있으면
        (예: CredentialAccess) 원본 첫 값을 유지해 폴백 경로로 남긴다.
    """
    normalized = [_TACTIC_ALIASES.get(t, t) for t in tactics]
    for t in normalized:
        if t in _UAV15_TACTICS:
            return t
    return normalized[0] if normalized else "Execution"
...
def _stride(display_name: str, description: str) -> list[str]:
    ...
    letters = [
        ltr
        for ltr, kws in _STRIDE_KEYWORDS.items()
...
def _load_properties(path: Path) -> dict[str, object]:
    """ARM 템플릿 JSON 에서 rule properties 를 추출한다.

    Args:
        path: AnalyticsRules 하위 S*/C*.json 경로.

    Returns:
        resources[0].properties 딕셔너리.

    Raises:
        ValueError: JSON 파싱 실패 또는 resources 구조가 기대와 다를 때
            (파일명 컨텍스트 포함).
    """
    try:
        ...
        raise ValueError(f"{path.name}: ARM 템플릿 구조 파싱 실패 — {exc}") from exc
...
def build(rules_dir: Path) -> list[dict[str, object]]:
    ...
    for sf in scenario_files:
        ...
        tactics: list[str] = (
            list(tactics_raw) if isinstance(tactics_raw, list) else ["Execution"]
        )
        entries.append(
            {
                "id": _slug_id(sf.stem),
...
def merge_with_existing(
    ...
    # 이번 스캔에 없는(소스 JSON 삭제/개명된) 기존 항목은 deprecated 로 캐리포워드
    # — deployed 로 남기면 존재하지 않는 룰이 탐지 커버리지로 영구 집계된다.
