...
class TestCatalogLoad:
    ...
    def test_gte_zero_rejected(self, tmp_path: Path) -> None:
        """`key>=0` 은 증거 부재를 증거로 만들 수 있어 거부한다."""
        bad = _VALID_YAML.replace(
            "prediction_probability>=0.6", "prediction_probability>=0"
        )
        with pytest.raises(HypothesisCatalogError, match="0보다 커야"):
            load_hypothesis_catalog(_write(tmp_path, bad))
