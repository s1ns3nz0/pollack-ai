...
class Thresholds:
    ...
    def violations(self, summary: PoamSummary) -> list[str]:
        ...
        if summary.high > self.high_max:
            msgs.append(f"high 미해결 {summary.high}건 > 허용 {self.high_max}건")
        if summary.medium > self.medium_max:
            msgs.append(f"medium 미해결 {summary.medium}건 > 허용 {self.medium_max}건")
        if summary.low > self.low_max:
            msgs.append(f"low 미해결 {summary.low}건 > 허용 {self.low_max}건")
        return msgs
...
def render_markdown(
    ...
    sorted_items = sorted(summary.open_items, key=lambda it: order.get(it.severity, 9))
    if not sorted_items:
        lines.append("(없음)")
    else:
        ...
        for it in sorted_items:
            ...
            lines.append(f"| {short_uuid} | {it.severity} | {it.status} | {it.title} |")
    lines.append("")
    return "\n".join(lines)
