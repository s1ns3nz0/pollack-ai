...
class TestCampaignChains:
    ...
    def test_chain_has_sequence(self) -> None:
        ...
        assert c1.sequence == ["S34", "S37", "S79", "S3"]
...
class TestCampaignDetector:
    ...
    def test_full_prefix_match(self) -> None:
        """C2 시퀀스 앞부분(S78,S33) 관측 → C2 매칭 + 다음 예상 S1."""
        matches = self._detector().detect(["S78", "S33"])
    ...
    def test_completed_campaign_no_next(self) -> None:
        ...
        matches = self._detector().detect(["S78", "S33", "S1"])
    ...
    def test_out_of_order_no_match(self) -> None:
        ...
        # S33 먼저, S78 없음 → C2 prefix(S78,..) 불일치
        matches = self._detector().detect(["S33", "S1"])
    ...
    def test_interleaved_history_matches_subsequence(self) -> None:
        ...
        # S34 → (무관 S99) → S37 → S79 : C1 prefix(S34,S37,S79) subsequence
        matches = self._detector().detect(["S34", "S99", "S37", "S79"])

        c1 = next(m for m in matches if m.chain_id == "C1")
        assert c1.matched == 3
        assert c1.next_expected == "S3"
    ...
    def test_next_expected_not_already_observed(self) -> None:
        ...
        C2=[S78,S33,S1]. history [S78,S1,S33] → S78→S33 매칭되나 S1(종단)이 S33 전에
        이미 발생 → next_expected=S1 은 거짓. 이미 본 시나리오는 예측하지 않는다.
        """
        matches = self._detector().detect(["S78", "S1", "S33"])
    ...
    def test_match_carries_severity(self) -> None:
        ...
        matches = self._detector().detect(["S34", "S37"])
    ...
    def test_only_first_element_no_match(self) -> None:
        ...
        matches = self._detector().detect(["S34"])

        # S34 은 C1/C4/C7 시작이지만 1개만으론 매칭 미확정
        assert matches == []
